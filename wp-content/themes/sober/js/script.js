	...
	sober.productImagesLightbox = function () {
		...
		$gallery.on( 'click', '.woocommerce-product-gallery__slider .woocommerce-product-gallery__image a', function ( e ) {
			...
			lightBox.listen( 'close', function() {
				var $iframe = $( lightBox.container ).find( '.sober-product-video__content iframe' ),
					$video = $( lightBox.container ).find( '.sober-product-video__content video' );
			...
			if ( $slides.length > 0 ) {
				$slides.each( function( i, el ) {
					...
					if ( $el.hasClass( 'sober-product-video' ) ) {
						item            = {
							html : $el.find( '.sober-product-video__wrapper' ).html()
						};
					} else {
						item            = {
							src  : large_image_src,
	...
	sober.productQuickView = function () {
		...
		sober.$body.on( 'click', target, function ( e ) {
			...
			$.post( ajax_url, {
				...
				var $productVideo = $( '.sober-product-video', $_product );

				if ( ! $productVideo.hasClass( 'sober-product-video--filetype' ) ) {
					$( 'video', $_product ).mediaelementplayer();
				}
