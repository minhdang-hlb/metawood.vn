...
class Sober_Shortcodes {
	...
	public static function collection_carousel( $atts ) {
		...
		foreach ( $collections as $collection ) {
			...
			$title = $collection['title'] ? '<h3 class="sober-collection-carousel__item-title">' . esc_html( $collection['title'] ) . '</h3>' : '';
			$button = $collection['button_text'] ? '<span class="sober-collection-carousel__item-button">' . esc_html( $collection['button_text'] ) . '</span>' : '';

			$list[] = '<div class="sober-collection-carousel__item"><a href="' . esc_url( $collection['url'] ) . '">' . $image . $title . $button . '</a></div>';
		}
		...
		return sprintf(
			'<div class="%s" data-free_mode="%s" data-loop="%s" data-autoplay="%s">%s</div>',
			esc_attr( implode( ' ', $css_class ) ),
			$atts['freemode'] ? 'yes' : '',
			$atts['loop'] ? 'yes' : '',
			intval( $atts['autoplay'] ),
			implode( '', $list )
		);
	}
