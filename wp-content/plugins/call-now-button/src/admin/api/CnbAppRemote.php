...
class CnbAppRemote {
    ...
    {
        global $cnb_user, $cnb_domain, $cnb_buttons, $cnb_domains, $cnb_coupon, $cnb_plans;
        $cnbAppRemote = new CnbAppRemote();
        $rest_endpoint = '/v1/wp/all/' . $cnbAppRemote->cnb_clean_site_url();
    ...
    public function get_button_for_condition($id)
    {
        ...
        foreach ($cnb_buttons as $button) {
            foreach ($button->conditions as $condition) {
                if ($condition->id === $id) return $button;
            }
        }
        return null;
    }
